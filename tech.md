    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "echo '' | grep -q 'rm -rf' && echo '{\"permissionDecision\": \"deny\", \"permissionDecisionReason\": \"禁止执行 rm -rf 命令\"}' && exit 2 || exit 0"
          }
        ]
      }
    ]
    ...
    "UserPromptSubmit": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "echo '{\"hookSpecificOutput\": {\"additionalContext\": \"当前 Sprint: 完成用户认证模块。最近错误: \"}}'"
          }
        ]
      }
    ]
